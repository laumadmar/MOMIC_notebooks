...
heatmap_function<-function(heatmapset,title){
    ...
    aheatmap(as.matrix(heatmapset) ,Rowv = TRUE , Colv = TRUE ,distfun = "euclidean", hclustfun = "average",scale = "row",main=title)
}
...
pca_function<- function(expression_df,clinicalset,variable) {
    ...
    # if there is not na, the try catch return a table and does not pritn the plot
    #pca <- prcomp(t(expression_df))
    #pca <- tryCatch({
    #    pca <- prcomp(t(expression_df))
    #    return(pca)
    #}, error = function(e) {
